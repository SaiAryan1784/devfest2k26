...
- The hero is a billboard: `HeroVideo` plays the silent 46 s loop in `public/video/` (encoded by `scripts/encode-hero-video.sh` from the gitignored source in `docs/reference/video/`; paths and the description line live in `EVENT.hero`) under two scrims, with its poster painted beneath as a real `<img>` so the LCP never waits for video. The `<video>` gets its `src` on the client only (720p below 768 px), never under reduced motion or Save-Data, and plays only while the hero is on screen. The copy is one left column; the lockup there is the loader's glide target. Every light mark on the page is `LightPipe` over a shape in `pipes.ts`; light colour comes from `PAL`/`SPECTRUM` in `slabs.ts`. The one paint-animated layer per view is the active track card's `pipe-run` stroke.
...
- Lockup geometry lives in `src/components/brand/lockup-paths.ts`; `Lockup` renders from it and the loader's `spectrum-field.ts` rasterises the same paths to read, per bar, the rows the mark occupies, so the lines draw exactly the mark that then sharpens over them. The loader (the shelf: `SpectrumStage`, `SpectrumField`, `spectrum-field.ts`) is drawn in code (Canvas 2D, transparent, DPR 1, no image files, no WebGL, no ReactBits); the mark appears only once the lines have drawn it, and the bars rush past the camera into the billboard. Tunables sit at the top of `spectrum-field.ts`.
