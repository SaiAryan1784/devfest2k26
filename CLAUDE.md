...
- Lockup geometry lives in `src/components/brand/lockup-paths.ts`; `Lockup` (static) and the loader's `LockupReveal` both render from it, so the two stay pixel-equivalent. A transform on an SVG child is not composited: anything that moves every frame inside the mark is an HTML overlay.
