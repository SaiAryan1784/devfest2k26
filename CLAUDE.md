...
- Lockup geometry lives in `src/components/brand/lockup-paths.ts`; `Lockup` renders from it everywhere. The loader is the ident (`IdentStage`, `IdentField`, `ident-field.ts`): the Netflix title card as a reference, on black. A line of light draws down the centre and splits into the site's spectrum of vertical stripes (12 px at a 30 px pitch on desktop, two fainter layers behind), the stripes light the rows of the mark so the lockup builds as a barcode before the real one sharpens over it, and at the cut the field rushes past the camera while the black dissolves onto the billboard's video, already playing under the gate. Drawn in code (Canvas 2D, transparent, DPR 1, no image files, no WebGL, no ReactBits, no filters), in passes with a scratch array; nothing full-screen uses a gradient fill or `imageSmoothingQuality: "high"` (measured: five blits' worth each). Tunables (pitch, widths, timings, push) sit at the top of `ident-field.ts`; the hold is `HOLD_MS` in `Loader.tsx`, the cut is `cut.ts`.
