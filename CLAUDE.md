...
- Lockup geometry lives in `src/components/brand/lockup-paths.ts`; `Lockup` renders from it everywhere. The loader is the blinds (`BlindsStage`, `BlindsField`, `blinds-field.ts`): thick frosted-glass strips over the billboard's video, drawn in code (Canvas 2D, transparent, DPR 1, no image files, no WebGL, no ReactBits, no backdrop-filter: the frost is the current video frame copied into a small buffer and stretched back up). The mark appears in front of the glass at the light pass and glides to the billboard; the canvas fades only once the strips have un-frosted and closed every gap, when it holds the same picture as the video beneath. Tunables (strip width, gap, timings, frost) sit at the top of `blinds-field.ts`; the hold is `HOLD_MS` in `Loader.tsx`, the cut is `cut.ts`.
