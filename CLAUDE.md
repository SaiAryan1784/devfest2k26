...
- The hero background is `Prism` and the loader is `PrismStage`, both drawn by `prism-field.ts` (Canvas 2D on a transparent canvas, DPR 1, no image, no filter, no WebGL): one white beam into the `{`, four colours out of the `}`. The hand-off is a swap between identical frames, gated on `useLoaderState.landed`; the loader's canvas follows the mark's glide by interpolating its rect with the glide's own ease, never by reading layout per frame. All tunables (rest angles, steering range, fan spread, rest brightness) sit at the top of `prism-field.ts`. Every light mark on the page is `LightPipe` over a shape in `pipes.ts`; all light colour comes from `PAL` in `slabs.ts`. The one paint-animated layer per view is the active track card's `pipe-run` stroke.
...
- Lockup geometry lives in `src/components/brand/lockup-paths.ts`; `Lockup` renders from it and the prism aims at its bracket boxes, so the light enters and leaves exactly where the mark's braces are. `Lockup`'s `sweep` prop is the only way the mark appears in stages: the loader's light front reveals it part by part, and nothing shows a part before the light has reached it. The loader is drawn in code (Canvas 2D, no image files, no WebGL, no ReactBits).
