...
- Lockup geometry lives in `src/components/brand/lockup-paths.ts`; `Lockup` renders from it everywhere. The loader is the frost (`FrostStage`, `FrostField`, `frost-field.ts`): the billboard's video seen through fogged glass for the whole sequence, with the Netflix-style light stripes in front as thick, spaced glass rods that fan out from the centre and fly past the edges at the cut. Drawn in code (Canvas 2D, DPR 1, no image files, no WebGL, no ReactBits, no backdrop-filter): the fog is the current video frame copied into a small buffer and stretched back up. Everything soft is composed in the 1/3-scale buffer and reaches the canvas in one bilinear blit; never a full-screen gradient fill or `imageSmoothingQuality: "high"` at full size (measured: five blits' worth). The mark appears in front of the glass at the light pass and glides to the billboard; the canvas fades only once the fog has cleared, when it holds the same picture as the video beneath. Tunables (rod width, spacing, timings, frost) sit at the top of `frost-field.ts`; the hold is `HOLD_MS` in `Loader.tsx`, the cut is `cut.ts`.
