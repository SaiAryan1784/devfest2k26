...
## 7. Loader spec: the blinds (21 Sep 2026; the shelf, Convergence, Ignition and the glass-panel gate it replaced are in the git history)

Components: `components/loader/Loader.tsx` (state machine, rendered in `layout.tsx` above `{children}`), `BlindsStage.tsx` (frame, captions, readout, the mark and its glide), `BlindsField.tsx` (the loader's canvas), `cut.ts` (the cut's timings), `useAssetProgress.ts` (real progress as three named tasks). The picture is `components/brand/blinds-field.ts`.

The idea: the designers' glass bars, made literal. Black, with thick strips of frosted glass through which the billboard's video shows blurred and tinted; the strips arrive from the centre, a light passes across them, the mark appears in front, and the blinds open into the full picture. One material, one idea, and it explains the billboard behind it.
...
The picture (Canvas 2D on a transparent canvas at device-pixel-ratio 1 over the gate's black backdrop; no image files, no WebGL, no filters, no backdrop-filter), keyed to drawn progress `p`. Strips 56 px wide with 40 px gaps on desktop (36 / 24 on phones), rounded ends, 6 % bleed. Each strip shows the slice of the video behind it: once per frame the visible part of the current frame (the `object-fit: cover` crop) is drawn into a buffer at 1/10 scale, and every strip blits its own column of that buffer back up with bicubic smoothing, which is the blur. Over it a milky tint (white 8 %), the spectrum tint by x from `slabs.ts` (10 %), a vertical shade at both ends, a 1 px specular on the left and a 1 px shade on the right:
1. **The first strip** (p 0.08 to 0.20). A hairline at the centre widens into the first strip as it slides in.
2. **The blinds** (0.15 to 0.55). The other strips arrive outward from the centre, one after another, sliding in from the top and the bottom in turn with a long ease-out and a small overshoot. Inside each, the frosted picture keeps moving; the strips breathe (±6 px, out of phase) once in place.
3. **The light pass** (0.55 to 0.75). A band in each strip's spectrum hue travels across the blinds left to right; as it crosses the centre the `{ DevFest }` mark fades in (0.60 to 0.70), white and crisp, in front of the glass with a soft glow behind it.
4. **The hold** (0.75 to 1.00). Readout to 100.
5. **The cut** (2.2 s). Over the first 1.2 s every strip un-frosts (the buffer's scale runs continuously from 1/10 to 1/2) and widens until the gaps close, centre first; the backdrop dissolves from 0.7 to 1.3 s, so no gap ever shows the sharp video; at 1.2 s the canvas fades over 0.4 s onto the real video, which now differs from it only by the last of the softness. At 0.99 s the mark glides onto the billboard's lockup (FLIP, 1.2 s, `[0.16,1,0.3,1]`), fading over its last 15%; `finish()` fires at 1.24 s so the copy rises while the mark lands; body released at 0.6 s; the gate unmounts on landing. The canvas loop stops itself 1.7 s into the cut.
