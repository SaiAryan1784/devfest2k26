...
## 7. Loader spec: the frost (21 Sep 2026; the blinds, the shelf, Convergence, Ignition and the glass-panel gate it replaced are in the git history)

Components: `components/loader/Loader.tsx` (state machine, rendered in `layout.tsx` above `{children}`), `FrostStage.tsx` (frame, captions, readout, the mark and its glide), `FrostField.tsx` (the loader's canvas and its three buffers), `cut.ts` (the cut's timings), `useAssetProgress.ts` (real progress as three named tasks). The picture is `components/brand/frost-field.ts`.

The idea: light through fogged glass. The whole screen is frosted glass over the billboard's video, so the picture is glimpsed the entire time, blurred and dimmed, people and stage lights moving behind the fog. In front, the Netflix-style light stripes the designers liked, thick and spaced: rods of clearer glass that draw down the centre, fan outward like records on a shelf, take a wave of light, and at the cut fly past the edges as the fog clears into the real video.
...
The picture (Canvas 2D at device-pixel-ratio 1 over the gate's black backdrop; no image files, no WebGL, no filters, no backdrop-filter), keyed to drawn progress `p`. The canvas is opaque for the whole sequence, so the backdrop under it fades as soon as the cut starts.
- **The fog.** Once per frame the visible part of the current video frame (the `object-fit: cover` crop) is drawn into a buffer at 1/3 scale, taken down to 1/10 (1/7 by the end of the hold), stretched back up to 1/3 with the bicubic filter and then to full size with the bilinear one: the two steps make the blur round rather than faceted, and the last step is the one full-screen blit per frame. Over it, in the 1/3 buffer before that blit: a black dim (50 %), a milky cast, a shade at the top and bottom for the captions, the light pass, the glow around every rod and the glow behind the mark.
- **The rods.** Base width 34 px on desktop (18 on phones), each 0.75 to 1.35 of it, resting at a spacing of 3.8 base widths jittered by 15 %, so about a dozen at 1440 and seven at 390; seeded, so a resize keeps the picture. Their ends run 4 % past the canvas, so no clip is needed. Through each rod the 1/3 copy of its own column, shifted sideways by 30 % of its width (the bend of the glass) and dimmed less than the fog; over it a milky tint and its spectrum hue from `slabs.ts` at 8 %; a specular running down it off-centre; a bright core in its hue that rests near the middle and breathes; a 1 px specular on the left, a 1 px shade on the right; and, in the fog behind it, a glow three widths wide in its hue.
1. **The rise** (p 0 to 0.10). The fog comes up out of black. Captions fade in.
2. **The first rod** (0.08 to 0.22). A hairline of light draws down from the centre of the screen and thickens into the first rod.
3. **The fan** (0.18 to 0.56). The other rods peel off the centre one after another, alternating sides, each sliding to its rest with a long ease-out while it widens from 40 % to full, the picture through it fixed to the page so the slide reveals what is behind. In place, each drifts ±4 px sideways out of phase.
4. **The light pass** (0.56 to 0.76). A wide faint band brightens the fog from left to right; every rod it crosses flares in its hot hue and its glow doubles. As it crosses the centre the `{ DevFest }` mark fades in (0.62 to 0.72), white and crisp, in front of the glass with a soft glow behind it.
5. **The hold** (0.76 to 1.00). The fog thins a little; the readout reaches 100.
6. **The cut** (2.2 s). The fly-through: over the first second every rod is pushed away from the centre with an ease-in (to 7× its offset, widening with the square root of the push) and fades from 0.35 to 0.9 s; the fog's scale runs from 1/7 to 1/2 and its dim to zero over 1.2 s (once the scale passes 1/3 the fog goes straight onto the canvas and the small buffer carries only the overlays); the backdrop under the canvas fades over the first 0.6 s; at 1.2 s the canvas fades over 0.4 s onto the real video, which now differs from it only by the last of the softness. At 0.99 s the mark glides onto the billboard's lockup (FLIP, 1.2 s, `[0.16,1,0.3,1]`), fading over its last 15%; `finish()` fires at 1.24 s so the copy rises while the mark lands; body released at 0.6 s; the gate unmounts on landing. The canvas loop stops itself 1.7 s into the cut.
...
Rules: transform and opacity only outside the canvas; the loader's canvas loop runs only while the stage is mounted; `Z.loader` is 55, under the film grain on purpose. Inside the canvas: everything soft is drawn in the 1/3 buffer, the full-size blit is bilinear, and nothing full-screen uses a gradient fill or the bicubic filter (see §22 for why).
