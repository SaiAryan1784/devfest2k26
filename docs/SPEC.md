...
## 7. Loader spec: the shelf (20 Sep 2026; Convergence, Ignition and the glass-panel gate it replaced are in the git history)

Components: `components/loader/Loader.tsx` (state machine, rendered in `layout.tsx` above `{children}`), `SpectrumStage.tsx` (frame, captions, readout, the mark and its glide), `SpectrumField.tsx` (the loader's canvas), `cut.ts` (the cut's timings), `useAssetProgress.ts` (real progress as three named tasks). The picture is `components/brand/spectrum-field.ts`. The mark's silhouette is rasterised from `components/brand/lockup-paths.ts`, the same data the static `Lockup` renders from.

The reference is the Netflix ident (CGI by Imaginary Forces, 2019): lines draw the N, take on many hues, and the camera zooms through them into the content. What we keep is the mechanism, in our alphabet: lines draw the mark, the mark holds, the lines take the site's spectrum and rush past the camera into the billboard.
...
- Progress is `useAssetProgress()`: `type` (`document.fonts.ready`), `stage` (`window.load`) and `video` (the billboard's loop reaches `canplay`, or 2.5 s pass, whichever first, so a slow line never stalls the opening). What is drawn is `min(progress, clock)`, the clock running linearly over 3.6 s from the moment the stage mounts.
...
- The hero is painted under the opaque gate until the loader commits to its sequence (`useLoaderState.showing`), so the browser records the LCP (the poster image) at first paint; it then drops to its hidden state under the gate and enters on `finish()`. The video starts playing at `showing`, under the gate, so it is already moving when the gate dissolves. On the skip path the hero never hides and the gate's dissolve is the entrance.
...
The picture (Canvas 2D on a transparent canvas at device-pixel-ratio 1, additive compositing; no image files, no WebGL, no filters), keyed to drawn progress `p`. Three layers of vertical bars at 6, 5 and 4 px pitch (about 890 bars at 1440, 240 on a phone); the front layer knows, per bar, the rows of the mark its column covers (`silhouette()`, one offscreen rasterisation of the lockup paths at its on-screen size):
1. **The lines** (p 0.10 to 0.42). Thin white lines sweep in from the left and right edges toward their columns, the outer ones first. Each stops at its column and lights only the mark's rows there, so `{ DevFest }` builds up as a barcode of light; columns with no mark keep a ghost of a line.
2. **The mark** (0.42 to 0.55). The crisp lockup fades in over its own barcode (same box, same geometry) and the lines flash once.
3. **The shelf** (0.55 to 0.80). Every bar extends to full height from the mark's rows outward (from the middle where there is no mark), the centre first, white at first and then the seven-hue spectrum from `slabs.ts` as dark glass with a hot band (a 1×256 sprite per hue, blitted). The two back layers appear during this beat.
4. **The push** (0.80 to 1.00). Every bar's x and width scale away from the centre, the front layer fastest; the field brightens.
5. **The cut** (1.5 s). The push runs on (scale 7 by 0.9 s, ease-in) and the field dissolves between 0.15 and 0.7 s; the backdrop dissolves 0 to 0.6 s under the transparent canvas, revealing the video already playing with the copy hidden; at 0.6 s the mark glides onto the billboard's lockup (FLIP, 0.9 s, `[0.16,1,0.3,1]`), fading over its last 15%; `finish()` fires at 0.85 s so the headline, description, CTAs and facts rise while the mark lands; body released at 0.6 s; the gate unmounts on landing. The canvas loop stops itself one second into the cut.
