...
interface SplashCursorProps {
  ...
  COLOR?: string;
  /** Patched: upstream had no way to pause the sim short of unmounting it
   *  (which leaks, see the cleanup note below). A ref bridges this into the
   *  running rAF loop without re-running the whole WebGL setup effect,
   *  mirroring PrismaticBurst's own pausedRef pattern. */
  paused?: boolean;
}
...
export default function SplashCursor({
  ...
  // Patched: see the `paused` prop's doc comment above.
  ...
  useEffect(() => {
    ...
    // Patched: capped at 1 (was the full, uncapped window.devicePixelRatio),
    // matching this project's WebGL DPR-1 rule (see PrismaticBurst) and
    // CLAUDE.md's "DPR capped at 1... or 1.5 max". On a Retina display the
    // uncapped version renders 4x the fragment-shader pixels of this cursor's
    // continuous, site-wide fluid sim.
    ...
    // Patched: upstream's requestAnimationFrame(updateFrame) below had no
    // way to be canceled on unmount, so the sim ran forever once a first
    // pointer event started it. rafId is tracked so the effect's cleanup can
    // actually cancel it, and pausedRef (see the `paused` prop above) lets
    // the caller go silent cheaply (a flag flip, used here to pause while
    // the hero is on screen or under reduced motion, per CLAUDE.md's WebGL
    // rule) instead of mounting and unmounting, which would otherwise leak a
    // WebGL context and this loop on every toggle.
    let rafId = 0;
    ...
    // Patched: every listener below is now a named function (upstream passed
    // anonymous arrows to addEventListener) so the cleanup at the end of this
    // effect can actually remove them; nothing else about the logic changed.
    ...
    // Patched: a real cleanup. Upstream had none, so this loop and every
    // listener above would outlive an unmount; not exercised in normal use
    // here (the mount is gated once per session, see CursorFx), kept for
    // correctness and so a future caller can safely unmount this component.
    return () => {
